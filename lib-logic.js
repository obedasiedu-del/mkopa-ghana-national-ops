...
// Splits a pasted "Depot, Model, In Stock, Returned" export into per-depot models
// objects, matching each row's depot text against the real depot list the same way
// the device-ledger bulk paste matches shop names to depots. Unlike that flow there
// is no Indirect Channel / Unrecognised catch-all here (depot stock only makes sense
// per real depot) -- a row that doesn't match a depot is reported as unmatched instead
// of being dropped or bucketed, so nothing is lost silently.
function parseDepotStockPaste(text, depots) {
    const lines = splitPasteLines(text);
    const depotIndex = buildDepotIndex(depots);
    const matchCache = {};
    function matchCached(depotText) {
        ...
        if (!(key in matchCache))
            matchCache[key] = matchDepotForShop(depotText, depotIndex);
    ...
    lines.forEach((line, idx) => {
        let cells = line.indexOf("\t") !== -1 ? line.split("\t") : line.split(",");
        cells = cells.map((c) => c.trim());
        if (idx === 0 && /^(depot|shop)/i.test(cells[0] || "") && /model/i.test(cells[1] || ""))
            return;
        ...
        if (!byDepot[code])
            byDepot[code] = {};
        if (!byDepot[code][model])
            byDepot[code][model] = { inStock: 0, returned: 0 };
