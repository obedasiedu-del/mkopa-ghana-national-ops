...
function BulkLedgerModal() {
    ...
    function getPasteText() {
        return rawTextRef.current || (textareaRef.current ? textareaRef.current.value : "");
    }
    ...
    return (React.createElement(Modal, { open: true, onClose: closeModal, xwide: true, title: "Upload Baseline \u2014 All Depots", footer: React.createElement("button", { className: "btn", onClick: closeModal }, "Cancel") },
        ...
        React.createElement("div", { style: { fontSize: 11.5, color: "var(--text-muted)", marginBottom: 10 } }, "Paste your full device export \u2014 every depot at once. Columns: Serial Number, Product, Shop Name, DSR Name, Device Age (days). Each row is matched to a depot by Shop Name and saved there; a shop name from a known indirect-channel partner (MTN, Telecel, Franko, izone, MCS, etc.) goes to the \"Indirect Channel\" bucket, and anything else unrecognised goes to the \"Unrecognised Shops\" bucket \u2014 nothing is dropped. Depots present in this paste have their device list replaced; others are left untouched. Every device starts \"In Stock\" \u2014 mark one Reallocated from its device table once it's recovered."),
        React.createElement("div", { className: "field-row" },
            React.createElement("div", { className: "field-label" }, "Paste device rows (all depots)"),
            React.createElement("textarea", { ref: textareaRef, className: "field-input", rows: 10, placeholder: "SN12345\tA07/64\tKasoa Main Shop\tKwame Mensah\t12\nSN67890\tA16/128\tCape Coast Shop\tAma Boateng\t3", defaultValue: "", onPaste: onPaste, onInput: onInput })),
        React.createElement("div", { className: "field-row" },
        ...
        React.createElement("div", { style: { fontSize: 12, margin: "4px 0 14px" } },
            ...
            !parsing && parsed && totalPasted > 0 && (React.createElement(React.Fragment, null,
                React.createElement("div", { style: { color: "var(--success)", fontWeight: 600, marginBottom: 4 } },
                    ...
                    " pasted device",
                    totalPasted === 1 ? "" : "s",
                    " ready to save, across ",
                    depotCodes.length,
                    " depot",
                    depotCodes.length === 1 ? "" : "s",
                    " (including Indirect Channel / Unrecognised where they apply)."),
                Object.keys(parsed.indirectCounts).length > 0 && (React.createElement("div", { style: { color: "var(--warning)", display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap", marginTop: 2 } },
                    React.createElement("span", null,
                        parsed.indirectRows.length,
                        " device",
                        parsed.indirectRows.length === 1 ? "" : "s",
                        " classified as Indirect Channel \u2014 shop name",
                        Object.keys(parsed.indirectCounts).length === 1 ? "" : "s",
                        ": ",
                        Object.keys(parsed.indirectCounts).slice(0, 8).join(", "),
                        Object.keys(parsed.indirectCounts).length > 8 ? ", …" : "",
                        "."),
                ...
                Object.keys(parsed.unrecognisedCounts).length > 0 && (React.createElement("div", { style: { color: "var(--warning)", display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap", marginTop: 2 } },
                    React.createElement("span", null,
                        parsed.unrecognisedRows.length,
                        " device",
                        parsed.unrecognisedRows.length === 1 ? "" : "s",
                        " classified as Unrecognised \u2014 shop name",
                        Object.keys(parsed.unrecognisedCounts).length === 1 ? "" : "s",
                        ": ",
                        Object.keys(parsed.unrecognisedCounts).slice(0, 8).join(", "),
                        Object.keys(parsed.unrecognisedCounts).length > 8 ? ", …" : "",
                        "."),
                ...
                parsed.skipped > 0 && React.createElement("div", { style: { color: "var(--text-faint)" } },
                    parsed.skipped,
                    " row(s) skipped (missing serial number).")))),
