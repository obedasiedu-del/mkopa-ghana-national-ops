...
export function LedgerModal({ depotCode: initialCode }) {
  ...
  function onDepotChange(code) {
    setDepotCode(code); setSetBy(""); setPasteText(""); setFilter("all"); setSearch("");
  }
  function saveBaseline() {
    const parsed = parsePastedDevices(pasteText);
    if (parsed.rows.length === 0) { toast("No valid rows found — check the pasted data."); return; }
    runAction(() => data.saveLedgerBaseline(depotCode, setBy, parsed.rows), "Baseline saved — " + parsed.rows.length + " device" + (parsed.rows.length === 1 ? "" : "s"))
      .then(() => { if (parsed.skipped > 0) toast(parsed.skipped + " row(s) skipped (missing serial number)"); setPasteText(""); });
  }
  ...
  return React.createElement(Modal, { open: true, onClose: closeModal, xwide: true, title: "Device Allocation Analysis", footer: React.createElement("button", { className: "btn", onClick: closeModal }, "Close") },
    ...
    React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", margin: "8px 0 10px" } },
      LEDGER_MODAL_CHIP_DEFS.map((c) => React.createElement("button", { key: c.key, className: "btn btn-sm" + (filter === c.key ? " btn-primary" : ""), onClick: () => setFilter(c.key) }, c.label))),
    rows.length === 0
      ? React.createElement("div", { style: { padding: "16px 0", color: "var(--text-faint)", fontSize: 12.5 } }, "No devices match.")
      : React.createElement("div", { style: { overflowX: "auto", border: "1px solid var(--border)", borderRadius: 8, maxHeight: 320, overflowY: "auto" } },
        React.createElement("table", null,
          React.createElement("thead", null,
            React.createElement("tr", null,
              React.createElement("th", null, "Serial"), React.createElement("th", null, "Product"), React.createElement("th", null, "Shop"),
              React.createElement("th", null, "DSR"), React.createElement("th", null, "Aging / Status"), React.createElement("th", null, "Actions"))),
